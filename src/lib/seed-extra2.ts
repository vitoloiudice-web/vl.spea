...
export async function generateAndSeedExtra2() {
  ...
  for (const bConf of batchConfigs) {
    ...
    // 5 Grandi, 5 Medie, 50 Piccole
    const distributions = [
      { f: "Grande >50M€" as Fascia, c: 5 },
      { f: "Media 10-50M€" as Fascia, c: 5 },
      { f: "Piccola <10M€" as Fascia, c: 50 }
    ];
    
    let compId = 300; // Starting from 300 to avoid ID collisions
    for (const dist of distributions) {
      for (let i = 0; i < dist.c; i++) {
        ...
        const c: Company = {
          id: `b${bConf.id}-extra2-${dist.f.charAt(0).toLowerCase()}${compId++}`,
          settore,
    ...
    // Save batch in chunks of 500 max (we have 60 so it's fine)
    ...
    await batch.commit();
    console.log(`Batch ${bConf.id} seeded with ${companies.length} extra2 companies.`);
    total += companies.length;
  }
  
  console.log(`Successfully generated and seeded ${total} extra2 companies across 10 batches.`);
}

generateAndSeedExtra2().then(() => process.exit(0)).catch((e) => { console.error(e); process.exit(1); });
