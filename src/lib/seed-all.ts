...
export async function generateAndSeedAll() {
  ...
  for (const bConf of batchConfigs) {
    ...
    // 12 Grandi, 20 Medie, 18 Piccole
    const distributions = [
      { f: "Grande >50M€" as Fascia, c: 12 },
      { f: "Media 10-50M€" as Fascia, c: 20 },
      { f: "Piccola <10M€" as Fascia, c: 18 }
    ];
    ...
    for (const dist of distributions) {
      for (let i = 0; i < dist.c; i++) {
        ...
        const c: Company = {
          id: `b${bConf.id}-${dist.f.charAt(0).toLowerCase()}${compId++}`,
          settore,
    ...
    await batch.commit();
    console.log(`Batch ${bConf.id} seeded with ${companies.length} companies.`);
    total += companies.length;
  }
  
  console.log(`Successfully generated and seeded ${total} companies across 10 batches.`);
}

generateAndSeedAll().then(() => process.exit(0)).catch((e) => { console.error(e); process.exit(1); });
